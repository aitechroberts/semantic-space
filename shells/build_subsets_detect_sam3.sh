...
# =============================================================================
# Build ablation subsets from an existing stride=1 detect_sam3+gdino run.
# Creates symlinked frame_data, then runs embed + build_map + oracle_finalize
# on each subset independently.
#
# Prerequisites: detect.py stride=1 segmentation_backend=detect_sam3
#                detector_type=gdino must have already completed.
#
...
#
# Usage:
#   SCENE=office0 bash shells/build_subsets_detect_sam3.sh
# =============================================================================
...
if [[ ! -d "$ORACLE_FD" ]]; then
    ...
    echo "Run detect.py with stride=1 segmentation_backend=detect_sam3 detector_type=gdino first."
    exit 1
fi

# --- Create subsets (symlinks only, no overwrites) ---
