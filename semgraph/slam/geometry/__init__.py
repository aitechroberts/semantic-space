...
from semgraph.slam.geometry.base import GeometryBackend, FrameContext


def get_geometry_backend(mode: str) -> GeometryBackend:
    """Factory that returns the appropriate GeometryBackend for *mode*."""
    if mode == "trajectory":
        from semgraph.slam.geometry.trajectory import TrajectoryBackend
        return TrajectoryBackend()
    elif mode == "gt_mesh":
        from semgraph.slam.geometry.gt_mesh import GTMeshBackend
        return GTMeshBackend()
    elif mode == "sparse":
        from semgraph.slam.geometry.sparse import SparseBackend
