...
Reads the intermediate map from build_map.py, optionally consolidates
captions via VLM, then saves the final map artifacts.

Standalone usage::

    python -m semgraph.stages.postprocess <hydra overrides>
"""
...
def consolidate_map_captions(
    ...
    from semgraph.utils.vlms.vlm_api import consolidate_captions
...
def finalize(
    ...
    if cfg.save_json:
        ...
        from semgraph.utils.general_utils import save_edge_json
        save_edge_json(
...
def main_standalone(cfg):
    ...
    if result is None:
        raise FileNotFoundError(f"Map not found at {paths['map']}")
