...
Provides:
  ...
  - ``SerializedDetection`` TypedDict — live in-memory per-detection record
...
def stage_paths(cfg: Any) -> dict[str, Path]:
    ...
    ``oracle``, ``variants``, ``assembled``, ``eval``, ``exp_out``.
    """
...
class SerializedDetection(TypedDict, total=False):
    """Numpy-only, live in-memory representation of a single detection.
...
# FrameDataRecord lives in semgraph.io.records (dataclass with to_arrays/from_arrays_and_metadata).
